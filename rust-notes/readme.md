...
// 因此需要加上一个<'a>生命周期标注，关联上参数和返回值，标注它们的生命周期是“一致”的。 好让借用检查器能够去检查longest的调用是否合法
...
* 当函数指定了泛型生命周期参数，函数可以接收带有任何生命周期的引用
...
```rust
// 此函数签名表示：x，y以及返回值这些引用，它们必须拥有相同的生命周期 'a
// x,y两个参数的存活时间必须不能短于 'a，返回值同样不能短于 'a 生命周期
// 这样的标注只是向借用检查器指出了可用于检查非法调用的一些约束而已
// 具体到这个例子，'a 具体代表的生命周期是那一段呢？ 'a 代表x，y中生命周期中短的那个(或者说是两者的交集)，
// 也即返回值生命周期也是x，y中短的那个生命周期
fn longest<'a>(x: &'a str, y: &'a str) -> &'a str {
    if x.len() > y.len() {
        x
    } else {
...
fn main() {
    ...
    let ret;
    {
        // 如果这里str2是定义成这样，则程序不会报错，原因是字符串字面值比较特殊，
        // 相当于整个程序的生命周期都是存活的，或者说这个内存引用在整个进程运行期间一定是有效的。
        // 其生命周期是 'static,也就是全局静态。
        // let str2 = "abc";
        let str2 = String::from("abc");

        // 此段代码编译时就会报错，根据生命周期标注，编译器就会推断出来，这里是不合法的
        // 因为虽然最后返回的是str1的引用，在外边大的作用域也是有效的，但是编译器并不知道这件事，
        // 它只关心通过生命周期标注，标注了返回值的生命周期是x，y中更小的那个，所以此行之后，rust就认为ret的引用失效了。
...
* 如果返回的引用没有指向任何参数，那么它只能引用函数内部创建的值：
...
// 要返回函数内部的变量，直接返回值就行，这样将发生所有权的转移，让调用者去释放变量
// 从这个例子，可以看出，根本上讲生命周期是用来关联不同参数和返回值之间的生命周期的，
// 一旦它们取得了某种联系，Rust就获得了足够的信息，来支持保证内存安全的操作。
// 并且阻止那些可能导致悬垂引用或者其他违反内存安全的行为。
...
* struct里可包括：
  ...
  - 引用：需要在每个引用上添加生命周期标注
```rust
// part是一个引用类型，所以需要标注生命周期，标注方式和泛型一样，在结构体名称后面的尖括号里面
// 下面这种写法意味着，part的生命周期必须要比结构体本身要长，因为此结构提对象存在的话，就会持有part对象的引用
...
fn main() {
    ...
    // first_sentence 生命周期比后面 i 对象长，就是ok的。可以编译通过
...
```rust
// 此函数参数是引用，但是没有标注生命周期参数，此函数也是合法的
// 早期版本（pre 1.0）需要标注生命周期，但现在已经不需要了
// 在一些可预测，存在一些明确的模式的情况，生命周期参数是可以省略的
// Rust团队将这些模式写入了编译器代码，让借用检查器在这些模式下，能够自动的对生命周期进行推导，而无需显式的标注
// 未来可能会有更多确定的模式会被添加到编译器中，以后需要手动标注生命周期的地方可能会越来越少
fn first_word(s: &str) -> &str {
    let bytes = s.as_bytes();
...
* 在Rust引用分析中所编入的模式称为 “生命周期省略规则”
  - 这些规则无需开发者来遵守
  - 它们是一些特殊情况，由编译器来考虑
  - 如果你的代码如何这些情况，那么就无需显式标注生命周期
* 生命周期省略规则不会提供完整的推断：
  - 如果应用规则后，引用的生命周期仍然模糊不清，则编译报错
  - 解决办法：添加生命周期标注，表明引用间的相互关系

#### 输入、输出生命周期
* 生命周期在：
  - 函数/方法的参数中：则被称为输入生命周期
  - 函数/方法的返回值：则被称为输出生命周期

#### 生命周期省略的三个规则
* 编译器使用3个规则在没有显式标注生命周期的情况下，来确定引用的生命周期
  - 规则1：应用于输入生命周期
  - 规则2、3：应用于输出生命周期
  - 如果编译器应用完3个规则之后，仍然有无法确定生命周期的引用，则编译器报错
  - 这些规则适用于fn定义和impl块
* 规则1：每个引用类型的参数都有自己的生命周期（1个参数的函数就拥有一个生命周期参数，2个参数的函数就拥有两个生命周期参数）
...
* 因为只有一个输入参数，也适用第二条规则：fn first_word<'a>(s: &'a str) -> &'a str {
应用完3条规则后，所有的引用都有了生命周期，编译器就可以继续分析代码，而无需程序员手动标注生命周期

* fn longest(x: &str, y: &str) -> &str {
* 应用第一条规则：fn longest<'a, 'b>(x: &'a str, y: &'b str) -> &str {
* 因为有两个输入参数，第二条规则不适用
* longest是一个函数不是方法，没有self参数，因此第三条规则也不适用
* 应用完三条规则后，编译器依旧不能计算出，返回值的生命周期，所以编译器就会报错
...
* struct字段的生命周期名：
  ...
  - 这些生命周期是struct类型的一部分
* impl块内的方法签名中：
  - “引用”必须绑定于struct字段引用的生命周期，或者“引用”是独立的也可以
...
// impl 和结构提后面的<'a>是不能忽略的
impl<'a> ImportantExcerpt<'a> {
    // 根据第一条省略规则，可以不为此方法添加生命周期标注
    fn level(&self) -> i32 {
        3
    }

    // 根据第一条规则，各自添加生命周期，根据第三条规则，为返回值添加self的生命周期
    // 因此所有参数的生命周期都是明确的，可以省略生命周期标注
